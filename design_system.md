...
- **Ledger Month Filter**: The Finance Ledger page should default to a monthly data window. Place the custom month selector beside CSV export actions, use previous/next arrow buttons for month movement, disable the next arrow on the current month, and make cards, activity charts, table rows, pagination, and CSV export reflect the selected month.
