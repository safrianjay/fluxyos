...
- **Dashboard/App Date Picker**: Use the shared `FluxyDateRangePicker` from `assets/js/date-range-picker.js` for dashboard date filters. It should default to the current month when used for ledger-style data, avoid separate Day/Month tabs and native calendar picker UI, support single-day and range selection inside the calendar, include Cancel + tertiary Reset + Apply actions, disable future dates, and keep scoped cards, charts, tables, pagination, and exports aligned to the selected period.
