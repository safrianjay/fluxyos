...
- **Ledger Date Filter**: The Finance Ledger page should default to the current month through a custom date-range calendar beside CSV export actions. Do not use a separate Day/Month tab or native calendar picker UI; the calendar itself must support single-day and range selection, previous/next period movement, disabled future dates, and scoped cards, activity charts, table rows, pagination, and CSV export.
