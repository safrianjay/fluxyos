...
- **Ledger Date Filter**: The Finance Ledger page should default to a monthly data window but also support a single-day view. Place the custom Month/Day selector beside CSV export actions, use previous/next arrow buttons for period movement, disable the next arrow on the current period, avoid native calendar picker UI, and make cards, activity charts, table rows, pagination, and CSV export reflect the selected period.
- **Ledger Breakdown Panels**: Status and Type breakdown lists should show about three rows before scrolling so the Ledger Activity card stays compact and does not create unused whitespace beside the volume chart.
