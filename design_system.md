...
- **Dashboard/App Entry Dates**: Entry drawers that write finance records include a date field defaulting to today, allow today or previous days only, and show an inline info warning above the sticky submit button when the selected date or CSV row dates are not today.
