...
|---|---|---|
| `chart_of_accounts` | `{code}` | `code, name, type (asset/liability/equity/revenue/expense), subtype, parent_code, normal_balance, is_active, currency, entity_id, opening_balance, created_at`, plus the policy flags `mappable`, `allow_manual_journal`, `allow_direct_transaction` and a `seed_version` stamp. Seeded idempotently by `seedChartOfAccounts()` from `CHART_OF_ACCOUNTS_SEED` (**45 accounts** — count it, do not quote it; this line has been wrong three times). Archive via `is_active`; **never deleted**. |
