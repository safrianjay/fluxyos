...
### Added (Member-path QA harness — verified live)
- **`tests/member-drilldown.spec.js`** — an invited-team-**member** browser test for the KPI drill-downs, now **passing against a real member session**. It resolves the QA owner's workspace id, sends a separate member account in through the invite link (`/login?invite=<email>&ws=<id>` → `healFromStoredInvite` accepts), then asserts on all four pages that the member resolves the shared owner workspace (`FluxyWorkspace.id !== memberUid`, not the `workspaces/{memberUid}` fallback), renders without permission errors, and sees the workspace's revenue. **Skips** until `.qa/firebase-test-member-account.md` exists (git-ignored); needs the member invited **from the QA owner account**. Provisioning in `docs/QA_TEST_ACCOUNT.md`. Closes the one blind spot the owner harness can't catch (for an owner `workspaceId === uid`, so member scoping bugs are invisible).
...
### Fixed (Bills "Record payment" button stuck on "Recording…")
- After successfully marking one bill paid, the next bill's Record-payment modal opened with a disabled "Recording…" button. `confirmPay()` reset the shared `#bill-pay-confirm` only in its `catch`; the success path closed the modal without restoring it, and the reused modal kept the stale state. Added `resetPayConfirmButton()` (restores label + `disabled=false` + `payInFlight=false`), called on modal **open and close** so the button is always clean regardless of how the previous attempt ended. Audited the parallel record flows (Invoices mark-paid, dashboard bank/budget setup, the budget-assignment drawer, the Add Transaction drawer) — all already reset correctly (via `finally` or on open); Bills was the only broken one. Regression test: `tests/bill-record-payment.spec.js`.
