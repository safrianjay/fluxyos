...
| POS integration | 🔭 | Admitted §5. **Reframed 2026-08-17 (`POS_READINESS.md`):** `commerce_orders` already posts revenue via `CM-ORDER-REV`/`FEE`/`REFUND`/`SETTLE` and has **never relieved stock** — there is no `CM-ORDER-COGS`. Per-sale costing is therefore a gap in a *shipped* pipeline, not a POS feature; building it on commerce orders first serves the current e-commerce base with no till involved. A POS is then one more connector. A first-party till is a separate, larger question (offline-first, shifts, cash drawer) and needs a rules trim first |
