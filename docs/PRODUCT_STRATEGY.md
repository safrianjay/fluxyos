...
| POS integration | 🔭 | Admitted §5. **Reframed 2026-08-17 (`POS_READINESS.md`):** `commerce_orders` already posts revenue via `CM-ORDER-REV`/`FEE`/`REFUND`/`SETTLE` and, until 2026-08-21, had **never relieved stock**. `CM-ORDER-COGS` shipped 2026-08-20 but nothing called it for a day, so the defect outlived the fix; it is now wired to the Inventory Overview. **POS Phase 0+1 shipped 2026-08-21** (`POS_IMPLEMENTATION_PLAN.md`). Per-sale costing is therefore a gap in a *shipped* pipeline, not a POS feature; building it on commerce orders first serves the current e-commerce base with no till involved. A POS is then one more connector. A first-party till is a separate, larger question (offline-first, shifts, cash drawer) and needs a rules trim first |
