...
| Point of Sale | ✅ First-party till + QR ordering; ◐ payment integration | Re-audited 2026-09-22 for the POS landing page: `assets/js/pos-service.js` and `assets/js/pos.js` provide tables, reservations, order lifecycle, modifiers, split table bills, manual payment recording and shifts; `order.html` + `netlify/functions/qr-*.js` provide QR ordering. `POS-SALE` and `CM-ORDER-COGS` connect sales and configured recipe stock costs to the ledger and outlet. Payment-provider processing and offline order/payment queues are **not built**. Access is eligibility-gated. See `docs/data-model/pos.md`, `tests/pos-table-bill.check.js`, `tests/customer-ordering.spec.js`. |
