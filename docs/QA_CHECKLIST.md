...
FluxyOS is an Intelligent Finance Operating System — a double-entry accounting kernel with operational modules feeding it, built as multi-page static HTML + vanilla JS + Firebase Firestore. See docs/PRODUCT_STRATEGY.md for the canonical positioning and scope. Every time a change or new feature is requested, this checklist is run to verify the affected area works correctly and that no other parts of the app have broken. The goal is a fast, consistent QA pass that can be done in the browser after each implementation.
...
| 12 | **Unnecessary white space — the six places it hides** | Item 11 catches gaps that are too SMALL. This catches gaps that are too BIG, which is the more common defect and the one no linter flags: empty space reads as "nothing here", and on a till it pushes the work below the fold. Walk all six on every page you changed: **header → content** (the canvas must start with the work, not with air under the topbar), **section → section**, **card padding**, **popup spacing** (a dialog sized for its content, not a fixed box with slack at the bottom), **form field alignment** (fields in a row share a baseline and fill their track — a lone field must span the row, not sit at half width beside nothing), and **button spacing**. The bar to clear is not "does it look tidy" but **"could a cashier work faster if this were tighter"**. Applies everywhere; it is non-negotiable on `pos.html`, which is operated standing up, at speed, on a 10" screen. |
...
| 15 | Ledger Visibility/Status/Type/Cash filters live behind a single **Filters** entry point in the page controls row (next to the date picker) that opens a two-pane **master-detail** popover: a left category rail + a right options pane ("Show only · <Category>"). Clicking a rail category swaps the right pane; selecting an option marks an orange dot on that category. **Status and Type are multi-select** (checkboxes, OR within the field; "All …" clears the group), **Visibility and Cash are single-select** (radios); a multi-select group counts as one applied filter and shows one "Status: N selected" chip. Selections are **staged** — a live "N applied" badge + "Results: N" preview update as you pick, but the table only changes on **Apply filters** (Reset/Cancel/✕/Escape/outside-click revert without committing). Once applied they default to "All"/"Active", narrow the table, intersect with date range + search + vendor filter, reset pagination to page 1, scope the CSV export, badge the Filters trigger with the active count, render removable chips above the table, and the two-pane panel fits cleanly at 375px without horizontal overflow. Status Breakdown and Type Breakdown panels are no longer rendered on the page |
...
| 17a | **Overview chart range regression** — set the period to **All Time**, then **Last Month**, across Net profit, Total income, Total expenses, Gross profit margin **and** Cash Flow. Confirm: (a) the **page never scrolls horizontally** — the sidebar must not overlap content (`document.documentElement.scrollWidth === clientWidth`); wide tracks scroll *inside* the card with a pinned Y-axis; (b) the timeline starts at the first period with data (no empty quarters padded out to today); (c) on short ranges the lines are not horizontally stretched and point markers stay round; (d) resize the window / collapse the sidebar and confirm the line charts repaint rather than stretch. See [DESIGN_SYSTEM.md §4a](../docs/DESIGN_SYSTEM.md). Automated: `npx playwright test tests/overview-charts.spec.js`. |
| 17b | **Overview chart reconciliation** — the chart headlines must equal the KPI cards above them for **every** period mode: Total income = Revenue, Total expenses = OpEx, the Net profit chart = the Net profit KPI, and the Expense breakdown donut total = OpEx. A chart that disagrees with its own KPI card is a blocking failure. |
| 17c | **Gross profit margin basis** — with **no** cost-of-revenue account mapped the card must show the "Cost of revenue not mapped" setup state, **never** a flat 100% margin (there is no COGS to subtract, so 100% would be fabricated). With one mapped, it plots a real margin. Verify both paths. |
| 17d | **Donut guards** — a workspace with many accounts/categories must keep the legend bounded (named rows + one counted `Other (N)` row), the centre total unclipped by the ring, and any zero/negative entry surfaced as a legend row rather than silently dropped. Run `node scripts/validate_palette.js "<slice hexes>"` if any slice colour changed. |
| 17e | **No per-chart AI entry point** — the Overview has exactly one AI surface (the right-rail AI Finance Summary). No chart card may carry an AI action, an "insights" button, or an `fx` affordance. |
| 17f | **Overview grid pairing** — at ≥1201px the AI rail card must be exactly as tall as the KPI board (not the reverse: the rail must never stretch the row), its content scrolls inside `.rail-scroll` with a thin subtle scrollbar, and the chart band spans the full canvas with left/right edges flush to the KPI board and the rail. At ≤1200px the layout stacks, the rail returns to natural height and stops scrolling internally — confirm it did **not** collapse to a sliver (the `position: absolute` reset). Order when stacked stays KPIs → charts → rail. |
| 17g | **Chart aspect** — generate/lengthen an AI summary and confirm the rail card height does not change. On line charts confirm point markers are circles, not ovals, at all three card sizes (full-width, 2-up, 3-up): the SVG `viewBox` must match rendered width **and** height. |
...
| 9 | **Net Profit** — headline and KPI strip agree with the Overview card; revenue-vs-expenses meter and the change bridge (previous net profit → revenue movement → expense movement → this period) render, and the bridge shows its documented empty state on All Time or when the previous window is empty. **Comparison by period is a diverging column chart, not a table**: Month/Quarter/Year each render columns around a shared zero baseline, every column is direct-labelled, profit columns sit above the line and losses below, no bar overflows the track, and the hover tooltip carries revenue, expenses, net profit, margin and change. Record scope (All / Revenue / Expenses) narrows the table and the subtitle count matches the rows |
| 10 | **Net Profit revenue-vs-expenses meter** — the panel is a meter, never a donut. In a profitable period the track shows a red **Spent** segment and a green **Kept** remainder that sum to the full track; every segment wide enough to hold text is labelled (red/green alone is indistinguishable under deuteranopia). In a loss period the track is fully **Spent** and a separate hatched **over-run** bar appears with "Over revenue by Rp… · N% over", and the footer reads "Spending is N× revenue this period". With no revenue at all it reads "All spend, no revenue". Nothing overflows its column at any period. There is **no in-page AI panel** on this page |
...
| 13 | Mark as Paid (bill drawer) opens the Record-Payment modal (payment date + cash impact; category & budget shown read-only, inherited). Confirm creates a linked Ledger expense (`type expense`, `linked_bill_id`), stamps the bill `payment_status "paid"` + `budget_impact_status "converted_to_actual"` + `linked_transaction_id`, writes the `bill.mark_paid` audit log — all one batch. The bill then shows "Paid", the button is hidden, and the drawer links "View in Ledger". Budget committed → actual on the same allocation (no double count). Paid is terminal. |
...
| 4 | Step 3 shows Auto split, labeled allocation fields, and a percent input wide enough for `100 %`; clicking Auto split evenly reallocates the current rows to the total budget |
| 5 | Dashboard's `OpEx vs Budget` KPI continues to read the same `total_budget` after the new flow saves; settings-budget.html's history table still shows the same active row |
...
| # | Check |
|---|---|
| 1 | Journal Register columns are Date / Journal # / Source / Description / Amount / Status / Actions — no Dr/Cr column; amounts are `Rp1.234.567` (no space, Inter `tabular-nums`, never mono) |
| 2 | Posted journals show `JE-YYYY-NNNNNN`; numbers are sequential per period-year with **no duplicates** across several new transactions/bills/invoices |
| 3 | Filters (search, status, type, source, account) intersect correctly; Clear resets them; period comes from the page date picker |
| 4 | Clicking a posted row opens `accounting-journal.html?id=…`; a draft row opens the editor (`accounting-journal-new.html?draft=…`) |
| 5 | Journal Detail shows header (number, status, date, period, type, source link, created/generated by, internal id, reference, description), balanced lines + totals, related ledger links, audit timeline, tax + AI placeholders |
| 6 | General Ledger rows and Trial Balance rows drill into Journal Detail (TB → GL → Journal → source) — no dead-end views |
| 7 | "New manual journal" button shows only for owner/admin/finance/accountant (hidden for viewer); rules still reject a viewer that bypasses the UI |
| 8 | Manual editor: Journal Number reads "Auto-generated on post" and is not editable; date drives the read-only posting period; subtype selectable |
| 9 | Save draft persists with **no** number and no ledger impact; a discarded draft burns **no** journal number (next post continues the sequence) |
| 10 | Post is disabled until debits == credits (>0); posting assigns the number, writes ledger balances, locks the entry, and redirects to its Detail page |
| 11 | A posted journal cannot be edited or deleted; Reverse posts a new numbered reversal into the open period and links `reversed_by_journal_id` (original shows "Reversed") |
| 12 | Editing/voiding a transaction still posts a reversal + repost, each taking the next sequential number (correction-in-current-period unchanged) |
| 13 | settings-team: Accountant appears in the role reference and the member/invite role pickers with its violet pill; assigning it works and persists |
| 14 | As an **invited** member (not owner), journals, numbers, manual journals, and counters all resolve (no hardcoded `users/`); run the §4 grep guard |
| 15 | Browser console clean (no CSP/CORS/404/Firebase errors) on the Register, Detail, and Manual editor pages |

**Rules deploy reminder:** `firestore.rules` changed (journal draft branching, `counters`, `accountant` role) — pushing main does **not** deploy rules. Run `firebase deploy --only firestore:rules` separately. Emulator suite: `firebase emulators:exec --only firestore,auth "node tests/accounting-kernel-rules-emulator-test.mjs"`.
...
| 29 | "Mark payment completed" (orange primary) shows ONLY on `open` invoices (incl. displayed-overdue); hidden on draft/void/paid. Edit invoice is white secondary — exactly one primary action in the detail header |
| 30 | Mark-paid modal shows invoice number, customer, Rp amount, and a payment-date picker (single-date, defaults today, future dates blocked); confirm creates EXACTLY ONE `users/{uid}/transactions` record: `type "income"`, `category "Revenue"`, `status "Completed"`, raw integer `amount` = invoice total, `vendor_name` = customer, `invoice_number` set, `timestamp` = picked date |
| 31 | After confirm: invoice has `status "paid"`, `paid_at`, and `linked_transaction_id` = the created transaction id; `invoice.mark_paid` audit log written; all three writes are one batch (a rules rejection leaves nothing half-written) |
| 32 | Paid invoice is terminal: Edit/Void/Mark-as-sent/Send-by-email/Mark-paid all hidden; rules block edits, item writes, void, and un-pay; memo/footer metadata edits are also blocked on paid. Detail + list show green Paid badge, Rp0 due, "Payment completed" activity line, and the PDF document shows a green "Paid" marker |
| 33 | The income transaction appears on the Ledger page with Rp dot-formatted amount and counts toward revenue KPIs; "Paid this month" invoice summary card includes the invoice |
