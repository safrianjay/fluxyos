...
| 15 | Ledger Visibility/Status/Type/Cash filters live behind a single **Filters** entry point in the page controls row (next to the date picker) that opens a stacked popover panel below it. Selections are **staged** — a live "N applied" badge + "Results: N" preview update as you pick, but the table only changes on **Apply filters** (Reset/Cancel/✕/Escape/outside-click revert without committing). Once applied they default to "All"/"Active", narrow the table, intersect with date range + search + vendor filter, reset pagination to page 1, scope the CSV export, badge the Filters trigger with the active count, render removable chips above the table, and the panel wraps/fits cleanly at 375px without horizontal overflow. Status Breakdown and Type Breakdown panels are no longer rendered on the page |
