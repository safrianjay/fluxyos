...
class DataService {
    ...
    async ensureWorkspace(uid, opts = {}) {
        ...
        const sellsAtCounter = typeof opts.sellsAtCounter === 'boolean' ? opts.sellsAtCounter : null;
        if (!wsExists) {
            await setDoc(wsRef, {
                owner_uid: uid,
                name: opts.name || null,
                ...
                ...(sellsAtCounter === null ? {} : { sells_at_counter: sellsAtCounter }),
                created_at: serverTimestamp(),
                updated_at: serverTimestamp()
            });
        } else if (ccy || country || category || holdsStock !== null || sellsAtCounter !== null) {
            ...
            if (sellsAtCounter !== null && cur.sells_at_counter === undefined) patch.sells_at_counter = sellsAtCounter;
    ...
    // POINT OF SALE
    //
    // The POS methods live in assets/js/pos-service.js and are mixed onto this
    // prototype at the bottom of this file. They were 1,011 lines here, in a file
    // every page imports; moving them makes the POS surface a bounded thing with
    // a checkable edge (tests/pos-service-boundary.check.js).
    //
    // `this` is still a full DataService inside them, so nothing about how they
    // are called changed — and the till bundle is NOT smaller yet, because
    // pos.html still loads this file. See the header of pos-service.js for the
    // dependency cone that has to be broken up before it can stand alone.
    // ═══════════════════════════════════════════════════════════════════════

    // Kept on the CLASS because callers use `DataService.POS_PAYMENT_METHODS`
    // and a mixin cannot supply a static. Delegates to the real list.
...
// ── Point of Sale ───────────────────────────────────────────────────────────
//
// Applied here rather than inside the class body because a class cannot be
// reopened. Runs once at module evaluation, before any instance exists, so
// every DataService carries the POS methods with `this` bound normally.
//
// assign, not defineProperty: these are plain methods with no getters, and
// keeping it boring means the only thing that changed is WHERE the code lives.
Object.assign(DataService.prototype, POS_METHODS);
