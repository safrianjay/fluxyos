...
const STATUS_BADGE = {
    ...
    at_risk:  { label: 'At Risk',  cls: 'bg-amber-50 text-amber-700 border border-amber-100' },
...
async function loadAndRender() {
    ...
    try {
        ...
        if (!state.selectedAnnualId && state.selectedBudgetId) {
            const selectedPeriod = state.periodBudgets.find(b => b.id === state.selectedBudgetId);
            if (selectedPeriod?.parent_budget_id) state.selectedAnnualId = selectedPeriod.parent_budget_id;
        }
...
function renderEmpty() {
    ...
    el('budget-total-hint').textContent = 'Set a period budget to define this working envelope.';
    el('budget-allocated').textContent = formatRp(0);
    el('budget-allocated-hint').textContent = 'Set a period budget to start allocating.';
    ...
    remainingTextEl.textContent = ' of period budget used';
    remainingTextEl.className = '';
    el('budget-unassigned').textContent = formatRp(0);
    el('budget-unassigned-hint').textContent = 'Set a period budget to build a baseline.';
...
function renderBudget(usage) {
    ...
    el('budget-allocated-hint').textContent = total > 0
        ? `${formatPercent(coveragePercent)} of period budget has a purpose.`
        : 'Set a period budget to start allocating.';
    ...
    remainingTextEl.textContent = remaining < 0
        ? ` of period budget used · ${formatRp(remaining)} over`
        : ' of period budget used';
    ...
    el('budget-unassigned-hint').textContent = total > 0
        ...
        : 'Set a period budget to build a baseline.';
...
function renderWorkspaceShell(budget, allocations) {
    renderWorkspaceHeader(budget || {}, allocations || []);
    renderPeriodBackLink(budget || null);
}

function renderPeriodBackLink(budget) {
    const link = el('period-back-link');
    if (!link) return;
    const mainId = budget?.parent_budget_id || state.selectedAnnualId || '';
    link.href = mainId ? `/budget.html?budgetId=${encodeURIComponent(mainId)}` : '/budget';
}
...
function renderAllocationMap(allocations, summary) {
    ...
    el('budget-allocation-summary').textContent = total > 0
        ...
        : 'Allocation map appears after a period budget amount is set.';
    ...
    bar.innerHTML = segments.map(seg => {
        ...
        return `
            <button type="button" class="group relative flex h-full items-center overflow-hidden border-0 px-2 transition-opacity hover:opacity-90 ${textColor}${cursor}"
                ...
                ${actionAttrs}
                role="img"
                aria-label="${escapeHtml(seg.label)} ${escapeHtml(formatPercent(seg.percent))} of period budget">
...
function renderBudgetAttention(allocations, summary) {
    ...
    if (total > 0 && allocated > total) {
        alerts.push({
            tone: 'danger',
            title: 'Allocation total exceeds period budget',
            body: `${formatRp(allocated - total)} is allocated beyond the period budget. Reduce allocations before using this plan for approvals.`
        });
    }
...
function handleBudgetExport() {
    ...
    const rows = [
        ...
        [],
        ['Metric', 'Amount'],
        ['Period Budget', Number(summary.total_amount) || 0],
...
function wizardValidationMessage() {
    ...
    if (budgetWizardState.step >= 3) {
        if (getWizardAllocatedTotal() > budgetWizardState.totalBudget) return `Allocations exceed the period budget by ${formatRp(getWizardAllocatedTotal() - budgetWizardState.totalBudget)}.`;
...
function renderBudgetWizard() {
    ...
    el('budget-wizard-progress').innerHTML = WIZARD_STEPS.map(step => `
        <div class="h-1 rounded-full ${step.id <= budgetWizardState.step ? 'bg-slate-950' : 'bg-gray-200'}"></div>
    `).join('');
...
function renderWizardChoiceButton(value, title, copy, activeValue) {
    ...
    return `
        <button type="button" data-wizard-choice="${escapeHtml(value)}" class="rounded-lg border px-4 py-3 text-left transition-all ${active ? 'border-[#EA580C] bg-white text-gray-900 ring-1 ring-[#EA580C]' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'}">
            ...
            <div id="budget-wizard-month-control" class="relative">
                ...
                <div id="budget-wizard-month-panel" class="budget-month-panel fixed z-[9999] hidden overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl">
                    ...
                    <div class="flex items-center justify-between border-t border-gray-100 bg-gray-50 px-4 py-3">
                        ...
                        <button type="button" data-budget-month-current class="rounded-lg px-3 py-2 text-[13px] font-bold text-[#EA580C] transition-colors hover:bg-gray-50">This month</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    return `
        <div class="space-y-5">
            ...
            <div class="rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-[13px] leading-6 text-gray-600">
                Set the total envelope before splitting it into allocations. You can rebalance allocations later while the period budget stays the control limit.
            </div>
        </div>
    `;
}
    ...
    return `
        <div class="space-y-5">
            ...
            <div class="rounded-lg border ${over ? 'border-red-200 bg-red-50' : 'border-gray-100 bg-gray-50'} px-4 py-3">
                ...
                <p id="budget-wizard-allocation-warning" class="${over ? '' : 'hidden'} mt-3 rounded-lg border border-red-200 bg-white px-3 py-2 text-[12px] font-medium text-red-700">${over ? `Allocations exceed the period budget by ${formatRp(over)}.` : ''}</p>
            </div>
        </div>
    `;
}
...
function renderTemplateCard(value, title, copy, disabled = false) {
    ...
    return `
        <button type="button" data-template="${escapeHtml(value)}" ${disabled ? 'disabled' : ''} class="relative rounded-lg border px-4 py-4 text-left transition-all ${active ? 'border-[#EA580C] bg-white ring-1 ring-[#EA580C]' : 'border-gray-200 bg-white hover:border-gray-300'} ${disabled ? 'cursor-not-allowed opacity-60' : ''}">
...
function mountWizardMonthPicker() {
    ...
    function renderPanel() {
        ...
        grid.innerHTML = monthNames.map((name, index) => {
            ...
            return `
                <button type="button" data-budget-month="${value}" class="rounded-lg border px-3 py-2 text-[14px] font-bold transition-all ${active ? 'border-[#EA580C] bg-white text-[#EA580C] ring-1 ring-[#EA580C]' : 'border-gray-100 bg-white text-gray-700 hover:border-gray-200 hover:bg-gray-50'}">
                    ${name}
                </button>
            `;
        }).join('');
...
function refreshWizardFooterAndComputed() {
    ...
    if (warningEl) {
        warningEl.textContent = over ? `Allocations exceed the period budget by ${formatRp(over)}.` : '';
