...
function posTodayKey() {
    const parts = Object.fromEntries(new Intl.DateTimeFormat('en-CA', {
        timeZone: posTimeZone(), year: 'numeric', month: '2-digit', day: '2-digit'
...
function metricCard(label, value, help, change, primary = false, money = false) {
    return `<article class="pos-overview-kpi${primary ? ' is-primary' : ''}">
        <div class="pos-overview-label">${esc(label)} <button type="button" class="metric-info" aria-label="${esc(label)}: ${esc(help)}" data-tooltip="${esc(help)}">?</button></div>
...
function overviewDonut(items, totalLabel, formatValue) {
    const rows = items.filter((item) => Number(item.value) > 0);
    const total = rows.reduce((sum, item) => sum + Number(item.value), 0);
    if (!total) return '<div class="pos-overview-empty">No data for this period.</div>';
...
function printOrderLabels(order) {
    ...
    const lines = picked.flatMap((i) => {
        ...
        return Array.from({ length: repeat }, () => `<section class="label"><header class="label-head"><span>${esc(table)}</span><strong>${esc(order.order_number || '')}</strong></header><main class="label-main"><h1>${esc(line.item_name)}</h1><div class="label-quantity"><span>QTY</span><b>×${quantity}</b></div>${modifiers ? `<p class="label-modifiers">${esc(modifiers)}</p>` : ''}${line.note ? `<p class="label-note"><strong>NOTE</strong>${esc(line.note)}</p>` : ''}</main><footer class="label-foot"><span>${esc(order.customer_name || order.channel || 'POS')}</span><span>${esc(order.id || '').slice(-6)}</span><time>${new Date().toLocaleString()}</time></footer></section>`);
    }).join('');
    ...
    el.innerHTML = `
        ...
        <section class="pos-modal pos-label-print" role="dialog" aria-modal="true" aria-labelledby="pos-label-print-title">
            ...
            <div class="pos-modal-body">
                ...
                <div class="pos-label-options"><div class="pos-field"><label>How should quantities print?</label><div class="pos-label-mode" role="group" aria-label="How quantities print"><button type="button" class="is-on" data-label-mode="quantity"><strong>Separate labels</strong><span>One label for every unit</span></button><button type="button" data-label-mode="line"><strong>One combined label</strong><span>One label showing the total × quantity</span></button></div></div><div class="pos-field"><label for="pos-label-copies">Copies of each</label><input id="pos-label-copies" type="number" min="1" max="20" value="1" inputmode="numeric"></div></div>
...
function writeOrderLabels(order, picked, mode, copies, w) {
    ...
    w.document.write(`<!doctype html><html><head><title>Order labels</title><style>@page{size:58mm auto;margin:2mm}*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;color:#000}.label{width:54mm;min-height:42mm;margin:0 0 2mm;padding:0;border:1px solid #000;page-break-inside:avoid;break-inside:avoid}.label-head{display:flex;align-items:center;justify-content:space-between;gap:4mm;padding:2mm 2.5mm;background:#000;color:#fff;font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase}.label-head strong{font-size:10px;white-space:nowrap}.label-main{padding:2.7mm 2.5mm 2mm}.label h1{margin:0 0 2.5mm;font-size:19px;line-height:1.04;letter-spacing:-.02em}.label-quantity{display:flex;align-items:baseline;gap:2mm;padding:1.4mm 0;border-top:1px solid #000;border-bottom:1px solid #000}.label-quantity span,.label-note strong{font-size:8px;font-weight:700;letter-spacing:.12em}.label-quantity b{font-size:25px;line-height:1}.label-modifiers{margin:2mm 0 0;font-size:10px;font-weight:700;line-height:1.3}.label-note{margin:1.8mm 0 0;padding:1.5mm 1.8mm;border-left:1.2mm solid #000;background:#f1f1f1;font-size:10px;line-height:1.35}.label-note strong{display:block;margin-bottom:.7mm}.label-foot{display:grid;grid-template-columns:1fr auto;gap:1mm;padding:1.7mm 2.5mm;border-top:1px dashed #000;color:#333;font-size:8px;line-height:1.3}.label-foot time{grid-column:1/-1;color:#555}@media print{.label{margin-bottom:2mm}}</style></head><body>${lines}<script>window.onload=()=>window.print()<\\/script></body></html>`);
    w.opener = null;
    w.document.close();
    ds.recordPosOrderLabelPrint(state.uid, order.id, { lineIds: picked.map((i) => all[i].line_id), copies, mode }).catch(() => {});
    */
