...
| 14 | Ledger date filter appears beside Download CSV, defaults to the current month, supports Month and Day modes, uses previous/next arrows, disables next on the current period, and updates ledger cards, activity charts, table rows, pagination, and CSV export scope |
| 15 | Ledger Status Breakdown and Type Breakdown panels show about 3 rows before scrolling, and the Ledger Activity card does not leave a large empty area under the volume chart |
